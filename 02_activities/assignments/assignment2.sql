...
SELECT 
    v.vendor_name,
    p.product_name,
    SUM(5 * cj.original_price) AS total_earnings
FROM 
    cross_joined cj
INNER JOIN vendor v ON cj.vendor_id = v.vendor_id
INNER JOIN product p ON cj.product_id = p.product_id
GROUP BY 
    v.vendor_name,
    p.product_name
ORDER BY 
...
VALUES (
    999, -- Example ID (make sure it doesn't duplicate)
    'Apple Pie',
    '10 inch',
    'unit',
    CURRENT_TIMESTAMP
